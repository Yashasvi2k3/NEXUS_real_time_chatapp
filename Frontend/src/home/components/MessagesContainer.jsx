...
// import notify from '../../assets/sound/notification.mp3'; // Commented out - file doesn't exist
...
const MessageContainer = ({ onBackUser }) => {
    ...
    useEffect(()=>{
      socket?.on("newMessage",(newMessage)=>{
        // const sound = new Audio(notify); // Commented out - sound file doesn't exist
        // sound.play();
          ...
          <>
              ...
              <div className='flex gap-2 md:justify-between items-center w-full'>
                ...
                <div className='flex justify-between mr-2 gap-2'>
                  <div className='self-center'>
                    <img className='rounded-full w-6 h-6 md:w-8 md:h-8 cursor-pointer' src={selectedConversation?.profilepic} />
                  </div>
            ...
            <div className='flex-1 overflow-auto'>
              ...
              {!loading && messages?.length > 0 && messages?.map((message) => {
                ...
                console.log('Message senderId type:', typeof message.senderId, 'Value:', message.senderId);
                console.log('Auth user ID type:', typeof authUser?._id, 'Value:', authUser?._id);
                
                // Temporary test - force first message to be "own" message
                const isOwnMessage = message === messages[0] ? true : String(message.senderId) === String(authUser?._id || authUser?.id);
                ...
                return (
                  <div className='text-white' key={message?._id} ref={lastMessageRef}>
                    <div className={`chat ${isOwnMessage ? 'chat-end' : 'chat-start'}`}>
                      ...
                      <div className={`chat-bubble ${isOwnMessage ? 'bg-sky-600' : 'bg-gray-600'}`}>
                        {message?.message}
                      </div>
            ...
            <div className='w-full rounded-full flex items-center bg-white'>
              ...
              <button type='submit'>
                ...
                <IoSend size={18}
                className='text-sky-700 cursor-pointer rounded-full bg-gray-800 w-8 h-auto p-1'/>
                }
              </button>
            </div>
            </form>
